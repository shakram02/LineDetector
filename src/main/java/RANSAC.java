...
public class RANSAC {
    ...
    public static List<Double> perform(List<Double> data_X, List<Double> data_Y, int num, int iter, double threshDist, double inlierRatio) {
        if (data_X.size() != data_Y.size()) {
            throw new IllegalArgumentException("Data must have same length");
        }
        int number = data_X.size();
        ...
        for (int i = 0; i < iter; i++) {
            ...
            List<Double> sample_X = new ArrayList<>();
            ...
            kLine.add(sample_X.get(1) - sample_X.get(0));
